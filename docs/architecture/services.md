...
Singleton-обёртки над внешними системами и наборами логики. Инициализируются один раз, доступны через геттеры из любого слоя.
...
Сервис — структурированная обёртка над чем угодно: внешней системой, библиотекой, набором связанной логики. Живёт в `pkg/services/`

Каждый сервис является singleton'ом, объекты объявлены внутри services/main.go, каждый сервис доступен через геттер из любого слоя.

Ключевое свойство: **каждый сервис независим от внешних зависимостей проекта**. Он не импортирует `internal/models` и не знает о других сервисах напрямую. Вместо этого использует примитивные типы или интерфейсы, описанные внутри себя. Это делает сервис переносимым между проектами без изменений. Таким образом каждый сервис создается с намерением сделать его не нагруженным бизнес логикой
